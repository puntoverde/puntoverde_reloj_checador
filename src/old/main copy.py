...
def init_model():
    ...
    DeepFace.verify(img1_path=imagen1,img2_path=imagen2,model_name="VGG-Face",detector_backend="mediapipe",threshold=.55)
    DeepFace.verify(img1_path=imagen1,img2_path=imagen2,model_name="Facenet512",detector_backend="mediapipe",threshold=.55)
...
def main(page: ft.Page):
    ...
    def fn_init_video(flag):        
        ...
        # image_mask_ovalo_inverso=cv2.addWeighted(frame,0.9,frame,0.1,0)
    ...
    def buscar(_):
        ...
        if data is not None:
            ...
            if(image[0] is not None):
                ...
                page.snack_bar = ft.SnackBar(content=ft.Text("No empleado con numero "+txt_nomina.value+" no tiene foto registrada",style=ft.TextStyle(size=30,color="#000000")),action="",bgcolor="#FFB300",duration=10000)
    ...
    def match(frame):
        ...
        if data is not None and data["verified"]: 
            ...
            data2=DeepFace.verify(img1_path=frame,img2_path=rostro_colaborador,model_name="Facenet512",detector_backend="mediapipe",threshold=.55)
    ...
    def guardar_rostro(frame):
        ...
        page.snack_bar = ft.SnackBar(content=ft.Text("Rostro Guardado Verifique de nuevo",style=ft.TextStyle(size=30)),action="exito",bgcolor="#5a98de",duration=2500)
        page.snack_bar.open = True            
        page.update()
            # audio=threading.Thread(target=gracias)
            # audio.start()
